...
async function returnOrderNumberV2(subject, companyObject, plainContent) {
  // look at subject and try with company prefix
  const subjectWithCompanyPrefix = await checkSubjectWithCompanyPrefix(
    subject,
    companyObject
  ).then((numberReturned) => {
  ...
  const subjectWithGenericPrefix = await checkSubjectWithGenericPrefix(
    subject
  ).then((numberReturned) => {
    subjectWithGenericPrefixResult = numberReturned
  })

  const contentWithCompanyPrefix = await checkSubjectWithCompanyPrefix(
    plainContent,
    companyObject
  ).then((numberReturned) => {
  ...
  const contentWithGenericPrefix = await checkSubjectWithGenericPrefix(
    plainContent
  ).then((numberReturned) => {
  ...
  return allResults
    .then((values) => {
      console.log(
        'got any?',
        subjectWithCompanyPrefixResult,
        subjectWithGenericPrefixResult,
        contentWithCompanyPrefixResult,
        contentWithGenericPrefixResult
      )
...
async function findCompanyByEmail(fields) {
  ...
  try {
    let company = await Company.findOne({
      where: { emailIdentifier: parsedCompanyEmail },
    })
    if (company === null) {
      let newCompany = await companiesController.internalCreate(
        'unconfirmed',
        parsedCompanyEmail
      )
      return newCompany
...
async function returnOrderDate(fields) {
  ...
  let getDateFromText =
    fromCompanyEmailGmailSpan.match(/Date:(.*)Subject/gi) || ''
...
async function parseEmailSender(plainContent) {
  ...
  let getEmailFromText = plainContent.match(
...
async function checkSubjectWithGenericPrefix(subject) {
  ...
  prefixes.some((regexPrefix) => {
    ...
    if (subject.match(new RegExp(regex, 'g'))) {
      ...
      const orderNumberArrayNoSpaces = orderNumberArray.map((str) =>
        str.replace(/\s/g, '')
      )
